	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Saiu mais 1 @${num.split('@')[0]}Já foi tarde😂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado por administrador ❌',
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511954819492@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome Do Bot* : ${me.name}\n*Número Do Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total De Pessoas Bloqueadas* : ${blocked.length}\n*O Bot está on* : ${kyun(uptime)}`
				...
				case 'dono':
					...
					buffer = await getBuffer(`https://www.google.com/search?q=Panda&safe=active&client=ms-android-samsung-gj-rev1&prmd=ivn&sxsrf=ALeKk02RVE5ddKwGl5Wsk_D0MZjhOUFcwQ:1612890409951&source=lnms&tbm=isch&sa=X&ved=2ahUKEwj1-cj_pN3uAhWhILkGHZhUCJsQ_AUoAXoECCgQAQ&biw=497&bih=936#imgrc=4EHIs9P8C8vxxM`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Este é meu dono*\n Ele é muito fofo\n Wa.me//+551195481-9492'})
					break
				case 'stiker':
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('Oque você está procurando?')
				...
				case 'tagall':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('𝙑𝘾 𝙀 𝙈𝙀𝙐 𝘿𝙊𝙉𝙊 𝙋𝙊𝙍 𝘼𝘾𝘼𝙎𝙊?')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('𝙋𝙍𝙊𝙉𝙏𝙊 𝙋𝘼𝙋𝘼𝙄 𝙇𝙄𝙈𝙋𝙀𝙄 𝙏𝙊𝘿𝙊𝙎 𝘾𝙃𝘼𝙏𝙎 :)')
					break
				case 'promote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						teks = '𝙈𝙀𝙈𝘽𝙍𝙊 𝘾𝙊𝙈𝙐𝙈 𝙋𝙍𝙊𝙈𝙊𝙑𝙄𝘿𝙊 𝙏𝙊 𝘿𝙀 𝙊𝙇𝙃𝙊 𝙀𝙈 🧐\n'
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'CONVERTIDO'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
